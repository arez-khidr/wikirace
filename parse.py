...
file_path = "/Users/arezkhidr/Desktop/enwiki-20241001-pages-articles-multistream.xml"
output_dir = "/Users/arezkhidr/Desktop/WikiData"
    ...
    try:
        for page in do_xml_parse(f, "{http://www.mediawiki.org/xml/export-0.11/}page"): 
            try:
                ...
                links = mw.parse(text).filter_wikilinks()
                title_links = [str(link.title) for link in links]

                filtered_text = wtp.parse(text).plain_text().replace("\n", "")

                if not filtered_text.split(" ")[0] == "#REDIRECT":
                    print(f"Creating file for: {title}")
                    file_name = sanitize_filename(title)
                    output_file_path = f"{output_dir}/{file_name}.txt"
                    ...
                    print(f"{title} is a redirect")
            except Exception as e:
                print(f"Error processing page {title}: {str(e)}")
                traceback.print_exc()

    except Exception as e:
        print(f"Failed to process the XML file: {str(e)}")
        traceback.print_exc()
